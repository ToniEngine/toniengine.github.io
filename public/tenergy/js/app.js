...
(function () {
  ...
  /* The home-screen exam library. Picking a card sets the engine's bank, so
     every downstream screen (topics, pool counts, deck) follows from it. */
  function buildLibrary() {
    ...
    Engine.banks().forEach(function (b) {
      ...
      card.type = 'button';
      card.className = 'exam';
      card.setAttribute('aria-pressed', b.id === Engine.currentBank() ? 'true' : 'false');
      card.innerHTML =
        '<span class="exam-check" aria-hidden="true"></span>' +
        '<span class="exam-kind">' + esc(b.kind) + '</span>' +
        '<span class="exam-name">' + esc(b.name) + '</span>' +
        '<span class="exam-blurb">' + esc(b.blurb) + '</span>' +
        '<span class="exam-stats">' + b.count + ' questions · ' + b.topics + ' topics</span>';
  ...
  /* Keep the library cards, the setup dropdown and the topic list agreeing. */
  function syncBank() {
    var current = Engine.currentBank();
    var host = $('library');
    if (host) {
      var cards = host.querySelectorAll('.exam');
      Engine.banks().forEach(function (b, i) {
        if (cards[i]) cards[i].setAttribute('aria-pressed', b.id === current ? 'true' : 'false');
      });
    }
    var sel = $('opt-bank');
  ...
  function boot() {
    if (!Engine.banks().length) {
      document.body.innerHTML = '<p style="padding:40px;font:16px sans-serif;color:#fff">' +
        'No question banks loaded — check that the js/questions*.js files are present.</p>';
      return;
    }
    buildLibrary();
    buildBanks();
